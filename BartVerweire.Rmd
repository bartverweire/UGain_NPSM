...
Er wordt een theorie mbt. het geheugen getest. In het onderzoek wordt het aantal woorden geteld dat een proefpersoon zich kan herinneren. Het experiment voorziet in 3 verschillende processen en 2 leeftijdscategorieën.
...
Er is een significant verschil tussen het Counting proces enerzijds, en de processen Imagery en Intentional anderzijds (van een significant verschil tussen Imagery en Intentional kunnen we via deze modellen niet spreken).
In het algemeen is er ook een significante invloed van de leeftijd : jongeren onthouden meer woorden dan ouderen (maar niet voor het Counting proces).
...
De kolom **p.value.adj** is de bonferroni-aangepaste p-waarde, en de kolom **significant** geeft aan of deze aangepaste p-waarde < 0.05. 
We zien dat het via de lineaire regressie bepaalde verschil steeds binnen het betrouwbaarheidsinterval van de Wilcoxon test valt.
...
Het Intentional proces voor ouderen is significant minder efficiënt dan wat de jongeren presteren in zowel het Imagery en Intentional proces.
...
Er zijn 2 onafhankelijke variabelen, namelijk Year en Monthly, maar we kunnen die combineren in 1 tijdsvariabele. We kunnen dus een aantal mogelijke modellen opstellen

* met Year als onafhankelijke variabele
* met Year en Monthly als onafhankelijke variabelen
* met Tijd (of numeriek : Year + Monthly/12) als onafhankelijke variabele

De functie anova geeft voor elk model aan of de coëfficienten significant zijn.
...
In het zuiver lineaire model zijn de coëfficiënten zwak significant. Maar, zoals visueel al duidelijk was, kunnen we er beter van uitgaan dat de afhankelijkheid niet-lineair is.
...
```{r, echo=FALSE}
result %>% 
  arrange(dist, sd.ratio, n.total, sample.ratio, test.type) %>%
...
We kunnen bv. ook de invloed bekijken van het type test, voor de verschillende combinaties van de andere parameters. De kolom winner bevat de test met de laagste p-waarde, en de kolom diff_pct het procentueel verschil tov. de kleinste waarde.
...
result %>% 
  ...
  mutate(winner = if_else(pooled < Welch, "pooled","Welch"),
         diff_pct = round(100 * (Welch - pooled) / if_else(pooled < Welch, pooled, Welch),1)) %>% 
  kable(caption="Invloed van test type onder verschillende scenario's")
```
## Conclusie
...
$$
\begin{aligned} \\
\mu_{L} &= e^{\mu_{N} + \frac{\sigma_{N}^2}{2}} \\
...
e^{\mu_{N}} &= \frac{\mu_{L}}{\sqrt{1 + \frac{\sigma_{L}^2}{\mu_{L}^2}}} \\
\end{aligned}
$$
...
\mu_{N} &= ln\left( \frac{\mu_{L}}{\sqrt{1 + \frac{\sigma_{L}^2}{\mu_{L}^2}}} \right) \\
\end{aligned}
$$
